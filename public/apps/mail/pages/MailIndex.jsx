...
export function MailIndex({ isSideNavPinned }) {
    ...
    function onMarkRead(mailToMark) {
        setMails(prevMails => {
            prevMails.find(mail => mail._id === mailToMark._id).isRead = true
            return [...prevMails]
        })
        ...
        mailService.save({ ...mailToMark, isRead: true })
            ...
            .catch(err => {
                console.log("err: ", err)
                setMails(prevMails => {
                    prevMails.find(mail => mail._id === mailToMark._id).isRead = false
                    return [...prevMails]
                })
    ...
    function onToogleStarred(mailToStar) {
        ...
        setMails(prevMails => {
            prevMails.find(mail => mail._id === mailToStar._id).isStarred = !boolean
            return [...prevMails]
        })
    ...
    function onRemoveMail(mailToRemove) {
        ...
        setMails(prevMails => prevMails.filter(mail => mail._id !== mailToRemove._id))
        if (!mailToRemove.removedAt && mailToRemove.sentAt) {
            ...
            mailService.remove(mailToRemove._id)
                .then(() => {
                    loadUnreadByStatus()
